...
class SteamAPIKeyMissing extends Error {
    problem: string;
    ...
    constructor(error: string, ...params: undefined[]) {
        ...
        if (Error.captureStackTrace) {
            Error.captureStackTrace(this, SteamAPIKeyMissing);
        }

        this.name = "SteamAPIKeyMissing";
        this.message = error;
        this.problem = error;
    }
}
...
class SteamUser {
    ...
    constructor(user: string) {
        ...
        this.API_KEY = config["steam"]["api_key"];
    ...
    async resolve_vanity() {
        ...
        if (is_none(this.API_KEY)) {
            throw new SteamAPIKeyMissing("Steam API key missing");
        }
        logger.info("Resolving vanity: " + this.user_data);
        const response = await this.request_api(
            "get",
            `ISteamUser/ResolveVanityURL/v1?key=${this.API_KEY}&vanityurl=${this.user_data}`
        );
        try {
    ...
    async fetch_info() {
        ...
        function _pick_avatar(steam_data: any) {
            let avatar = null;
        ...
        const raw_results_hell = await Promise.all([
            this.request_api(
                "get",
                `ISteamUser/GetPlayerSummaries/v2?key=${this.API_KEY}&steamids=${this.user_data}`
            ),
            this.request_xml("profiles/" + this.user_data.toString() + "/games"),
            this.request_api(
                "get",
                `IPlayerService/GetSteamLevel/v1?key=${this.API_KEY}&steamid=${this.user_data}`
            ),
            this.request_api(
                "get",
                `ISteamUser/GetFriendList/v1?key=${this.API_KEY}&steamid=${this.user_data}`
            ),
            this.request_api(
                "get",
                `ISteamUser/GetPlayerBans/v1?key=${this.API_KEY}&steamids=${this.user_data}`
            ),
        ]);
        logger.info("Finished requesting, parsing results!");
        const raw_results: { [key: string]: any } = {};
        // @ts-ignore
        const user_data: SteamUserData = {};
        ...
        for (const [ident, result] of Object.entries(raw_results)) {
            logger.info(`Parsing "${ident}" data...`);
            if (ident === "summary") {
                const summaries = result["response"]["players"][0];
                user_data["id"] = summaries["steamid"];
                user_data["name"] = summaries["personaname"];
                const avatar = _pick_avatar(summaries);
                if (is_none(avatar)) {
                ...
                const profile_data: string = summaries["profileurl"];
                if (profile_data.endsWith("/")) {
                    profile_data.slice(0, -1);
                }
                let extracted_vanity_id = null;
                ...
                try {
                    user_data["state_detail"] = getValueFromKey(
                        state_map,
                        summaries["personastate"].toString(),
                        "Unknown"
                    );
                } catch (e) {
                ...
                try {
                    user_data["visibility_detail"] = getValueFromKey(
                        visibility_map,
                        summaries["communityvisibilitystate"].toString(),
                        "Unknown"
                    );
                } catch (e) {
...
export async function fetch_steam_game_info(app_id: string): Promise<[SteamGameData | object, string]> {
    ...
    const qparam = { cc: "id", l: "en", appids: app_id };
    ...
    const raw_steam_data = json_data[app_id];
    if (!raw_steam_data["success"]) {
        return [{}, "Failed fetching that appID from Steam."];
    }
    const steam_data = raw_steam_data["data"];
    // @ts-ignore
    const parsed_data: SteamGameData = { price_data: {} };
    ...
    const collect_screenshot: any[] = [];
    if (hasKey(steam_data, "screenshots")) {
        steam_data["screenshots"].forEach((screenies: any) => {
    ...
    if (hasKey(steam_data, "price_overview")) {
        const price_data = steam_data["price_overview"];
        // @ts-ignore
        const price_map: SteamGamePriceData = {};
...
export async function do_search_on_steam(query_search: string): Promise<SteamGameSearch[]> {
    ...
    const final_results: SteamGameSearch[] = [];
    results["items"].forEach((res: any) => {
        // @ts-ignore
        const data: SteamGameSearch = {};
        data["id"] = res["id"];
        data["title"] = res["name"];
        if (hasKey(res, "price")) {
            const current_price: string = res["price"]["final"].toString();
            const main_price = parseInt(current_price.slice(0, current_price.length - 2)).toLocaleString(
                "en-US",
                {
                    minimumFractionDigits: 2,
                }
            );
            const additional = current_price.slice(current_price.length - 2);
            const parsed_price = main_price.slice(0, main_price.length - 3).replace(",", ".");
...
export async function do_steamdb_search(
    query: string,
    add_dlc = false,
    add_app = false,
    add_music = false
): Promise<[SteamDBSearchData[], string]> {
    const headers = {
    ...
    const query_param = {
        ...
        attributesToRetrieve:
            "name,objectID,tags,multiplayerCategories,vrCategories,oslist,categories,appType,userScore,developer,publisher,price_us,releaseDate,releaseYear",
        query: query,
    };
    ...
    try {
        ...
        response = await axios.get(SDB_ALGOLIA, {
            params: query_param,
            headers: headers,
        });
    } catch (error) {
    ...
    function setToUnknown<T extends string>(data: T | undefined | null): T {
        if (is_none(data)) {
            return "Unknown" as T;
        }
        return data;
    ...
    const final_results: SteamDBSearchData[] = [];
    game_list.forEach((game_res) => {
        // @ts-ignore
        const data: SteamDBSearchData = {};
        const gtype = getValueFromKey(type_map, game_res["appType"], "unknown");
        ...
        const platform_tick = { windows: false, mac: false, linux: false };
...
export async function fetch_steam_user_info(
    user_id_or_vanity: string
): Promise<[SteamUserData | object, string]> {
    const logger = MainLogger.child({ fn: "fetch_steam_user_info" });
    const steam_u = new SteamUser(user_id_or_vanity);
    try {
        const steam_res = await steam_u.fetch_info();
