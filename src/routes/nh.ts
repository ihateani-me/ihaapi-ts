...
NHRoutes.get("/t/:media_id/:img_num", (req, res) => {
    ...
    const image_path = `${req.params.media_id}/${req.params.img_num}`;
    nhImagePathProxy(image_path, true)
        .then(([img_buf, mimes]) => {
            if (!Buffer.isBuffer(img_buf)) {
                res.status(404).json(img_buf);
            } else {
...
NHRoutes.get("/image/:doujin_id/:page_num", (req, res) => {
    ...
    const doujin_id = req.params.doujin_id;
    const page_num = parseInt(req.params.page_num);
    if (isNaN(page_num)) {
        res.status(400).json({ status_code: 400, message: "page number are incorrect" });
    } else {
        nhImageProxy(doujin_id, page_num)
            .then(([img_buf, mimes, stat_code]) => {
                if (!Buffer.isBuffer(img_buf)) {
