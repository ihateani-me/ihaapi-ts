...
// Original script by wll8
// https://github.com/wll8/redoc-try
// Modified to fit my website
...
function trySwagger(cfg) {
  ...
  // the newest version are missing fullApi class, add it.
  $(".tryBtn").parent().addClass("fullApi");
  $(".tryBtn").click(function (event) {
    ...
    if (fullApi.trim() === "") {
        fullApi = $tryBtn.parent().text().replace(cfg.tryText, '').trim();
    }
    console.log(fullApi);
